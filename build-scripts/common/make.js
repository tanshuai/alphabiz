...
const doMake = async () => {
   ...
   */
  // execSync(`cp -r "${packageDir}/" "${resolve(__rootdir, 'out')}/"`)
...
const doPostmake = () => {
  const outDir = resolve(__rootdir, 'out/make')
  const destDir = resolve(__rootdir, `out/installers/${version}`)
...
if (process.argv.includes('--make')) {
  ...
  if (platform === 'win32') {
    ...
    const languages = existsSync(i18nDir)
      ? readdirSync(i18nDir).filter(i => i.match(/^\w{2}-\w{2}$/)).map(i => i.toLowerCase())
